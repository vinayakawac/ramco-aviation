 ...
 *
 * Two kinds of shell can be under `userData.airframe`: the procedural one, whose painted
 * surfaces all share the memoised `mat.skin()` instance, and a loaded GLB, whose
 * materials are unique per mesh and are tagged by airframeModel.js instead. The tagged
 * ones carry their own base colour because they are not all the same to begin with.
 */
export function setAirframeLit(aircraft, lit) {
  ...
  aircraft.userData.airframe.traverse((o) => {
    ...
    for (const m of Array.isArray(o.material) ? o.material : [o.material]) {
      if (!m) continue;
      if (m === mat.skin()) m.color.setHex(target);
      else if (m.userData.airframeSkin) {
        if (lit) m.color.setHex(target);
